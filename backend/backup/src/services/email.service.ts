...
export class EmailService {
  ...
  async sendEmail(to: string, subject: string, html: string) {
    try {
      ...
      if (this.transporter) {
        ...
        console.log('Ì≥ß Email would be sent (development mode):')
        console.log('To:', to)
    ...
    const html = `
      ...
      <html>
        ...
        <body>
          <div class="container">
            <div class="header">
              <h1 style="margin: 0;">Welcome to ${config.appName}!</h1>
            </div>
            <div class="content">
              ...
              <p>Please login at <a href="${config.appUrl}/login">${config.appUrl}/login</a></p>
              
              <p style="margin-top: 30px;">
                <a href="${config.appUrl}/login" class="button">Login Now</a>
              </p>
              
              <hr style="border: none; border-top: 1px solid #ddd; margin: 30px 0;">
              
              <p><strong>${config.appName} Management</strong></p>
              <p>Email: ${config.email.from}</p>
  ...
  async sendPaymentNotification(payment: any, tenant: any) {
    const subject = `New Payment Received - ${config.appName}`
  ...
  async sendReceiptEmail(tenant: any, receipt: any) {
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p style="margin-top: 30px;">
                <a href="${config.appUrl}/api/receipts/${receipt.id}/download" class="button">Download Receipt</a>
              </p>
  ...
  async sendPaymentStatusNotification(tenant: any, payment: any) {
    const subject = `Payment ${payment.status === 'VERIFIED' ? 'Verified' : 'Rejected'} - ${config.appName}`
  ...
  async sendBalanceNotification(tenant: any, balance: number) {
    const subject = `Balance Reminder - ${config.appName}`
          ...
          <div class="container">
            ...
            <div class="content">
              ...
              ${balance > 0 ? `
                ...
                <p style="margin-top: 30px;">
                  <a href="${config.appUrl}/tenant/payments/new" class="button">Make Payment</a>
                </p>
              ` : `
              ...
              <p><strong>${config.appName} Management</strong></p>
              <p>Email: ${config.email.from}</p>
              <p style="color: #666; font-size: 12px;">This is an automated message from ${config.appName} System. Please do not reply to this email.</p>
            </div>
          </div>
        </body>
      </html>
  ...
  async sendMaintenanceUpdate(tenant: any, maintenance: any, updateType?: string) {
    const subject = `Maintenance ${updateType ? updateType.charAt(0).toUpperCase() + updateType.slice(1) : 'Update'} - ${config.appName}`
