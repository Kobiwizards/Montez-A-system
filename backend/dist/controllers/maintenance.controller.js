...
class MaintenanceController {
    constructor() {
        this.createRequest = async (req, res) => {
            try {
                if (!req.user) {
                ...
                const maintenanceRequest = await prisma_1.prisma.maintenanceRequest.create({
                    data: {
                        tenantId: tenant.id,
                        title,
                        description,
                ...
                await this.emailService.sendMaintenanceUpdate(tenant, maintenanceRequest, 'created');
        ...
        this.getAllRequests = async (req, res) => {
            try {
                const { page = 1, limit = 20, status, priority, apartment, sortBy = 'createdAt', sortOrder = 'desc', } = req.query;
                ...
                const [requests, total] = await Promise.all([
                    prisma_1.prisma.maintenanceRequest.findMany({
                        where,
                        skip,
                        take,
                        orderBy: {
                    ...
                    prisma_1.prisma.maintenanceRequest.count({ where }),
                ]);
        ...
        this.getRequestById = async (req, res) => {
            try {
                const { id } = req.params;
        ...
        this.updateRequestStatus = async (req, res) => {
            try {
                ...
                const existingRequest = await prisma_1.prisma.maintenanceRequest.findUnique({
                    where: { id },
                    include: {
                        tenant: true,
                    },
                ...
                // Update request
                const request = await prisma_1.prisma.maintenanceRequest.update({
                    where: { id },
                    data: updateData,
                });
                // Log update
        ...
        this.updateRequest = async (req, res) => {
            try {
                if (!req.user || req.user.role !== 'ADMIN') {
                    return res.status(403).json({
                        success: false,
        ...
        this.deleteRequest = async (req, res) => {
            try {
                if (!req.user || req.user.role !== 'ADMIN') {
                    return res.status(403).json({
                        success: false,
                ...
                // Delete request
                await prisma_1.prisma.maintenanceRequest.delete({
                    where: { id },
                });
                // Log deletion
                await this.auditLogService.log({
        ...
        this.getTenantRequests = async (req, res) => {
            try {
                if (!req.user) {
                    return res.status(401).json({
                        success: false,
